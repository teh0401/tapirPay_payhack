...
BEGIN
  ...
  SET 
    environmental_points = GREATEST(0, environmental_points - (points_to_redeem * environmental_points / total_points)::INTEGER),
    social_points = GREATEST(0, social_points - (points_to_redeem * social_points / total_points)::INTEGER),
    governance_points = GREATEST(0, governance_points - (points_to_redeem * governance_points / total_points)::INTEGER),
...
-- Add unique constraint on user_id for user_esg_points if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'user_esg_points_user_id_key'
  ) THEN
